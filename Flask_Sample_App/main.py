...
class Posts(db.Model):
    ...
    postby = db.Column(db.String(20),nullable=False)
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=box_title,slug=slug,content=content,subhead=subhead,img_file=img_file,date=date,postby=postby)
                db.session.add(post)
                db.session.commit()
...
def uploader():
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            return "Bamm!! file uploaded successfully"
